#pragma once
// duplicate code in BPCommonTypes, needed for ACES types
...
struct optional_: private boost::noncopyable {
    ...
    explicit optional_() {
        ...
        if (!extract<boost::optional<T> >(object()).check()) {
            ...
            converter::registry::push_back(&convertible, &constructor, type_id<boost::optional<T> >(), &conversion::get_pytype);
        }
    }
